...
def _get_django_db_settings(database='default'):
    ...
    try:
        ...
        try:
            ...
            print("Failed to find database settings for {0}".format(database))
            sys.exit(1)
    env['db_port'] = db_port
    env['db_host'] = db_host
...
def _mysql_exec_as_root(mysql_cmd, root_password=None):
    ...
    try:
        mysql_call += ['--port=%s' % int(env['db_port'])]
...
def db_exists(db_user, db_pw, db_name, db_port, db_host):
    db_exist_call = ['mysql', '-u', db_user]
    
    if db_pw != '':
        db_exist_call += ['-p'+db_pw]
    db_exist_call += ['--host=%s' % db_host]

    try:
        db_exist_call += ['--port=%s' % int(db_port)]
    except (TypeError, ValueError):
        pass 
...
def db_table_exists(table_name, db_user, db_pw, db_name, db_port, db_host):
    table_list_call = ['mysql', '-u', db_user]
    
    if db_pw !='':
        table_list_call += ['-p'+db_pw]
...
def dump_db(dump_filename='db_dump.sql', for_rsync=False, tables=[], options=[], mode='w'):
    ...
    if not db_engine.endswith('mysql'):
        ...
        sys.exit(1)
    dump_cmd = ['/usr/bin/mysqldump'] + options + ['--user='+db_user, '--password='+db_pw,
                '--host='+db_host]
    try:
        port = int(db_port)
